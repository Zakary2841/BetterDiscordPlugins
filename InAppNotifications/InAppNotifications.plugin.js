...
const config = {
      ...
      load() {
        BdApi.UI.showConfirmationModal(
          ...
          `The library plugin needed for AQWERT'sPluginBuilder is missing. Please click Download Now to install it.`,
          {
            ...
            onConfirm: () => {
              request.get(
                "https://raw.githubusercontent.com/Zakary2841/BetterDiscordPlugins/refs/heads/main/InAppNotifications/0PluginLibrary.plugin.js",
                (error, response, body) => {
                  if (error)
                    return electron.shell.openExternal(
                      "https://betterdiscord.net/ghdl?url=https://raw.githubusercontent.com/Zakary2841/BetterDiscordPlugins/refs/heads/main/InAppNotifications/0PluginLibrary.plugin.js"
                    );
  ...
  : (([Plugin, Library]) => {
      ...
      const ChannelTypes = {
          ...
          PRIVATE_THREAD: 12,           // a temporary sub-channel within a GUILD_TEXT channel, viewable only by those invited
      ...
      class plugin extends Plugin {
        ...
        onMessage({ message }) {
          ...
          if (!author.globalName || author.globalName.toLowerCase() === author.username.toLowerCase()) {
            authorName = author.discriminator === "0" || "0000" ? author.username : author.tag;
          }
          if (channel.guild_id) {
